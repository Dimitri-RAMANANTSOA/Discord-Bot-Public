...
class Buttons(discord.ui.View):
    ...
    @discord.ui.button(label="J'accepte le règlement",style=discord.ButtonStyle.green)
    async def green_button(self,interaction: discord.Interaction, button: discord.Button):
        ...
        if role not in interaction.user.roles:
            ...
            await interaction.response.send_message(f"Merci d'avoir accepté le règlement\nN'hésite pas à aller choisir ta faction dans {role_channel.mention}", ephemeral = True)
        else: await interaction.response.send_message(f"Tu as déjà accepté le règlement", ephemeral = True)
...
------------------------------------------------------------\n
Bienvenue {member.mention} sur le serveur Discord d'Indomitus, vidéaste de la chaine YouTube éponyme. Tu es invité à prendre connaissance des règles du serveur dans le salon {codex_channel.mention} et de les approuver pour avoir accès au reste du serveur. Suite à cela tu pourras discuter et partager dans les différents salon de discussion du serveur. 
Pour tout renseignements ou requêtes tu peux t'adresses à un **{moderators_role.name}**.\n
**L'empereur protège**\n
------------------------------------------------------------\n
